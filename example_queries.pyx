...
cdef class SimpleSearch:
    ...
    cdef void *sets[2] #Pointers to stuff coming from the DB: array 1 and set 2 (we don't need 0)
    cdef int set_types[2]  #Constants: 1 means we fetch a set  2 means we fetch an array
